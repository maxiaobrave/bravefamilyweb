...
export const siteConfig = (key, defaultVal = null, extendConfig = {}) => {
  ...
  // 对显式本地配置做更高优先级覆盖，避免被 Notion 配置表中的旧值盖掉
  if (key === 'HOME_BANNER_IMAGE') {
    val = getValue(BLOG.HOME_BANNER_IMAGE, getValue(val, siteInfo?.pageCover))
  }

  if (key === 'AVATAR') {
    val = getValue(BLOG.AVATAR, getValue(val, siteInfo?.icon))
  }
  ...
  if (!val) {
    ...
    switch (key) {
      case 'HOME_BANNER_IMAGE':
        val = getValue(BLOG.HOME_BANNER_IMAGE, siteInfo?.pageCover) // 显式配置的首页大图优先于Notion封面
        break
      case 'AVATAR':
        val = getValue(BLOG.AVATAR, siteInfo?.icon) // 显式配置的头像优先于Notion图标
        break
      case 'TITLE':
